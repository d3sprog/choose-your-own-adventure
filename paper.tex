\documentclass[anonymous,sigplan,screen]{acmart}
...
\todo{Add argument for why formalism is needed: give a concrete example (e.g.\ the uniqueness/reconstructibility gap) that informal reasoning would miss.}
...
We start by reviewing five instances of the interaction pattern.

\todo{Clarify the two Gamma models upfront at the start of Section~\ref{sec:examples}: Sec.~4.1 generates direct operation calls; Sec.~4.2 models the actual Gamma, which hides operations behind member accesses.} In all of them, an editor
...
construct a program $e$, is it possible to recover the original sequene\todo{typo: sequene→sequence} of choices through which
...
becuase\todo{typo: becuase→because} the resulting proof would consist of a sequence formed by the selected tactics.
...
sequence of choices, because the choices are items of the member chain. \todo{Sketch decode: e.g.\ $\ident{decode}(e.\iota_1.\ldots.\iota_n) = (\iota_1,\ldots,\iota_n)$ — don't just assert it's easy.} A slight caveat is that
...
that can be applied to $X_2$ in order to reconcile its structure with the sturcture\todo{typo: sturcture→structure} of $X_1$.
...
\todo{Formalise the Coq tactic-sequence case (state = sequence of tactics) alongside the Idris term-based case — only Idris is formally modelled; Coq is described in prose.} \todo{Discuss Programming by Navigation [lubin-2025-pbn]: maps directly onto CYOA (step set = choices, Strong Soundness = Correctness, Strong Completeness = Completeness); note reconstructibility is what CYOA adds.}
